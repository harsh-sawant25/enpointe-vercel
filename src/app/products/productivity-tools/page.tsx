...
const FeatureItem = ({ children }: { children: React.ReactNode }) => (
    <div className="flex items-center gap-3 text-[0.88rem] text-white/80 py-2 border-b border-white/5 last:border-b-0 leading-snug">
...
const ProductCard = ({ tag, title, description, features, className = "" }: { tag: string, title: string, description: string, features?: string[], className?: string }) => (
    <div className={`group relative border border-white/10 bg-[#111111] transition-all duration-300 hover:border-white/30 overflow-hidden flex flex-col h-full ${className}`}>
        <div className="p-10 flex flex-col flex-grow">
            <div className="text-[0.6rem] tracking-[0.2em] uppercase text-white/40 mb-3 border border-white/10 inline-block px-2 py-1 self-start">
                {tag}
            </div>
            <h3 className="font-syne text-[1.5rem] font-bold mb-4 text-white uppercase">{title}</h3>
            <p className="text-[0.9rem] text-white/50 leading-[1.6] mb-8 font-light flex-grow">
                {description}
            </p>
            {features && (
...
const Page = () => {
    return (
        <div className={`${syne.variable} ${dmSans.variable} font-dm-sans bg-[#080808] text-[#f5f5f0] selection:bg-white/20 min-h-screen`}>
            {/* Page Hero */}
            ...
            <section className="px-5 md:px-20 py-24 bg-[#080808]">
                <div className="max-w-7xl mx-auto">
                    ...
                    <p className="text-base md:text-[1.1rem] text-white/50 font-light leading-[1.75] max-w-[800px] mb-16">
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid md:grid-cols-2 gap-12 lg:gap-24 mt-8 items-center">
                        <div>
                            ...
                            <p className="text-base md:text-[1.1rem] text-white/50 font-light leading-[1.75]">
                        ...
                        <div className="flex flex-wrap gap-3 content-start">
                            {[
                                ...
                                <span
                                    key={i}
                                    className="text-[0.75rem] md:text-[0.8rem] tracking-[0.05em] text-white/70 border border-white/10 px-4 py-2.5 bg-white/5 hover:bg-white/10 transition-colors"
                                >
                                    {tag}
                                </span>
                            ))}
